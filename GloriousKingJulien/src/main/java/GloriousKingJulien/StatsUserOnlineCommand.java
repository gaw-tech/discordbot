...
public class StatsUserOnlineCommand {
	...
	public void run(MessageReceivedEvent event) {
		...
		if (content.startsWith(this.prefix + "onlinestats")) {
			MessageChannel channel = event.getChannel();
			String server = event.getGuild().getId() + "online";
			...
			if (content.contains("all")) {
				DrawStats.drawOnlineStats(server);
				...
				if (minutes < 5)
					return;
				DrawStats.drawLastMinutesOnlineStats(server, minutes);
				graphdescription = "int the last " + minutes + " minutes.";
			}

			EmbedBuilder eb = new EmbedBuilder();
			eb.setTitle("Crappy graph!");
			eb.setDescription("Graph of how many users were online " + graphdescription);
			eb.setImage("attachment://onlinestats.jpg");
			...
			channel.sendMessage(eb.build()).addFile(file, "onlinestats.jpg").queue();
